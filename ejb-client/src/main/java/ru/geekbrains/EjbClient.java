...
public class EjbClient {
    ...
    public static void main(String[] args) throws IOException, NamingException, ExecutionException, InterruptedException {
        ...
        ToDoServiceRemote todoService = (ToDoServiceRemote) context.lookup("ejb:/main/ToDoServiceImpl!ru.geekbrains.service.ToDoServiceRemote");

        todoService.findAll()
                .forEach(todo -> System.out.println(todo.getId() + " " + todo.getName() + " " + todo.getPrice() + " " + todo.getQuantity() + " " + todo.getDescription()));
